...
@section('content')
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        <header class="mb-8">
            <h1 class="text-4xl font-bold text-indigo-800 mb-2">{{ __('Consultation Requests') }}</h1>
            <p class="text-lg text-gray-600">{{ __('Manage and view all consultation requests') }}</p>
        </header>

        <div class="bg-white shadow-lg rounded-lg p-6 mb-8">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <form action="{{ route('consultation_requests.index') }}" method="GET" class="flex w-full md:w-auto">
                    <input type="text" name="search" placeholder="{{ __('Search') }}" value="{{ request('search') }}" class="flex-grow md:flex-grow-0 border border-gray-300 rounded-l px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            @foreach($consultationRequests as $request)
                <article class="bg-white rounded-lg shadow-md overflow-hidden flex flex-col transform hover:scale-105 transition-transform duration-200 {{ $request->status == 'pending' ? 'border-yellow-500' : ($request->status == 'accepted' ? 'border-green-500' : 'border-red-500') }}">
                    ...
                    <div class="p-4 flex-grow">
                        <p class="text-gray-600 mb-2"><span class="font-semibold text-indigo-600">{{ __('Priority') }}:</span> {{ $request->priority->name }}</p>
                        <p class="text-gray-600 mb-2"><span class="font-semibold text-indigo-600">{{ __('Category') }}:</span> {{ $request->category->name }}</p>
                        <div class="text-gray-800 mb-4 line-clamp-3">{!! $request->description !!}</div>
                    </div>
                    <footer class="bg-gray-50 px-4 py-3 mt-auto">
                        <div class="flex justify-between items-center">
                            <a href="{{ route('consultation_requests.show', $request->id) }}" class="text-indigo-600 hover:text-indigo-800 font-medium">{{ __('View') }}</a>
                            <a href="{{ route('consultation_requests.edit', $request->id) }}" class="text-indigo-600 hover:text-indigo-800 font-medium">{{ __('Edit') }}</a>
